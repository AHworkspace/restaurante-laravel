...
{
    public function index(){return view('formatos-empaque.index',['formatos'=>FormatoEmpaque::with('unidadMedida')->orderBy('nombre')->get(),'unidades'=>UnidadMedida::reales()]);}
    ...
    private function validar(Request $request,?FormatoEmpaque $formato=null):array{return $request->validate(['nombre'=>['required','string','max:80',Rule::unique('formatos_empaque')->ignore($formato)],'descripcion'=>['nullable','string','max:255'],'unidad_medida_id'=>['nullable',Rule::in(UnidadMedida::idsReales())],'es_granel'=>['nullable','boolean'],'activo'=>['nullable','boolean']])+['es_granel'=>$request->boolean('es_granel'),'activo'=>$request->boolean('activo')];}
}
