...
<template id="linea-template">
    <div class="border-bottom pb-3 mb-4 linea">
        <div class="row g-2">
            ...
            <div class="col-lg-2 col-md-4"><label>Unidad de medida comprada</label><select class="form-select unidad-compra" required><option value="">Seleccionar</option>@foreach($unidades as $u)<option value="{{ $u->id }}">{{ $u->nombre }} ({{ $u->abreviatura }})</option>@endforeach</select><small class="text-muted">Kg, litro, unidad, arroba, etc.</small></div>
        ...
        <div class="row g-2 mt-1">
            <div class="col-lg-4 col-md-6"><label>Precio de lo comprado (Bs)</label><input type="number" step="0.0001" min="0" class="form-control precio" required><input type="hidden" class="unidad-precio"><input type="hidden" class="factor-precio"><small class="text-muted">Si elegiste caja como formato, este precio es por caja; si no, es por la unidad de medida.</small></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function agregar() {
        ...
        fila.querySelector('.presentacion').addEventListener('change', () => {
            ...
            const configuracion=presentaciones[fila.querySelector('.presentacion').value];
            if(configuracion?.stock){fila.querySelector('.unidad-compra').value=configuracion.stock;fila.querySelector('.unidad-contenido').value='';}
            recalcular(fila);
        });
        ...
        fila.querySelector('.unidad-compra').addEventListener('change', () => {
            ...
            if(!fila.querySelector('.cantidad-contenido').value){fila.querySelector('.unidad-contenido').value='';}
            recalcular(fila);
        });
